export const createMainNavigation = (filteredFilms) => {
  //todo может как-то упростить?
  ...
  const {count: favoriteFilmsCount} = findFavoriteFilter;
  const {count: historyFilmsCount} = findHistoryFilter;
  const {count: watchListFilmsCount} = findWatchListFilter;
